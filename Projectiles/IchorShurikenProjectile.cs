...
{
    public class IchorShurikenProjectile : ModProjectile
    {
        ...
        {                                                           // sound that the projectile make when hiting the terrain
            {
				...
				if (Main.rand.Next(10) == 0){
					Item.NewItem((int)projectile.position.X, (int)projectile.position.Y, projectile.width, projectile.height, (mod.ItemType("IchorShuriken")));
				}
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            {
                target.AddBuff(BuffID.Ichor, 90, true);
            }
        }
